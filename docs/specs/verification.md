...
- The system SHALL expose explicit static, read-only runtime, and disruptive verification intents. `(pending #3)`
- Static verification SHALL validate configuration and the stack contract without requiring running containers. `(pending #3)`
- Runtime verification SHALL remain non-destructive and SHALL fail when any mandatory runtime domain is unavailable, unknown, or skipped. `(pending #3)`
- Optional or intentionally unselected capabilities MAY report not applicable without weakening mandatory evidence. `(pending #3)`
...
- Verification output SHALL distinguish pass, warning, failure, not-applicable, and unknown outcomes. `(pending #3)`
- Verification SHALL emit bounded boolean evidence without public IPs, private addresses, API keys, passwords, forwarded-port values, or secret-bearing URLs. `(pending #3)`
