...
- The system SHALL preserve separate core and acquisition phases. Acquisition credentials or capabilities SHALL NOT block core setup. `(pending #3)`
...
- Acquisition reconciliation SHALL require current VPN health and fail-closed evidence before starting any risky service. `(pending #3)`
- Provider secrets SHALL enter through a controlling terminal, SHALL NOT be accepted as command arguments or JSON input, and SHALL NOT appear in structured output or setup state. `(pending #3)`
